...
class TestListBedCountsHandler(tornado.testing.AsyncTestCase):
  ...
  def setUp(self):
    ...
    self.request = tornado.httpserver.HTTPRequest(
       method='GET', uri=dashboard.ListBedCountsHandler.ROUTE,
    ...
    rid = self.handler.db.add_region(self.admin_id, store.Region(
        name='myregion'))
    self.icuid = self.handler.db.add_icu(self.admin_id, store.ICU(
        name='iuc1', region_id=rid))
