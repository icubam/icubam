...
class SQLiteDBTest(absltest.TestCase):
  ...
  def test_bedcount_where(self):
    with tempfile.TemporaryDirectory() as tmp_folder:
      ...
      for i in range(10):
        for j in range(10):
          sqldb.update_bedcount(
            i + 1, "test", 10, 9, 8, 7, 6, 5, 4, update_ts=cur_time + j
          )
      ...
      history = sqldb.get_bedcount(get_history=True)
      self.assertEqual(len(history), 110)
      history = sqldb.get_bedcount(icu_ids=[1], get_history=True)
      self.assertEqual(len(history), 11)
      history = sqldb.get_bedcount(icu_ids=[1, 2, 3], get_history=True)
      self.assertEqual(len(history), 33)
      new_max_time = sqldb.get_bedcount(
        icu_ids=[1, 2, 3], max_ts=max_time - 3, get_history=True
      )["update_ts"].max()
      self.assertEqual(new_max_time, max_time - 4)
